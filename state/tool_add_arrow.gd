...
@onready var arrow_scene = preload("uid://bt0o6olaydwjq")
@onready var arrow_holder = $"../../papper/arrowholder"
...
func instantiate_link(box_a: PanelContainer, box_b: PanelContainer): #NOTE been arrowyfied
	var link = arrow_scene.instantiate()
	link.start_box = box_a
	link.end_box = box_b
	
	box_a.arrows.append(link)
	box_b.arrows.append(link)

	arrow_holder.add_child(link)
...
func check_if_link_pair_doesnt_exists(sb : PanelContainer, eb : PanelContainer) -> bool: #checks if the two boxes has a link between them
	for L: Arrow in sb.arrows:
		if L.start_box == sb and L.end_box == eb:
			return false
	return true
